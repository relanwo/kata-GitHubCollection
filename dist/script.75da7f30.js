...
function getApiData(value) {
  if (value) {
    return fetch("https://api.github.com/search/repositories?q=".concat(value, "&per_page=5")).then(function (response) {
      if (response.ok) {
      ...
      items.forEach(function (item) {
        ...
        element.addEventListener("click", function () {
          ...
          name.innerText = "Name: ".concat(item.name);
          element.appendChild(name);
          var owner = document.createElement("p");
          owner.classList.add("repository__owner");
          owner.innerText = "Owner: ".concat(item.owner.login);
          element.appendChild(owner);
          var stars = document.createElement("p");
          stars.classList.add("repository__stars");
          stars.innerText = "\u2606 ".concat(item.stargazers_count);
          element.appendChild(stars);
          var cross = document.createElement("button");
          cross.classList.add("repository__cross"); // cross.innerText = `${}`;
